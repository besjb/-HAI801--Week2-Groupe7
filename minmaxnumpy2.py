...
def check_victory(grid, player):
    return (np.any(np.all(grid == player, axis=0)) or 
            np.any(np.all(grid == player, axis=1)) or 
            np.all(np.diag(grid) == player) or 
            np.all(np.diag(np.fliplr(grid)) == player))

def is_game_over(grid):
    return ('-' not in grid) or check_victory(grid, 'X') or check_victory(grid, 'O')
